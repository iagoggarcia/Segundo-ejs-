...
void buscar(abin A, tipoelem inf, abin *aux) {
    tipoelem E;
    if (!esVacio(A) && *aux==NULL){ // si el árbol no está vacío y el nodo no se ha encontrado todavía
        leer(A, &E); // leo el elemento del nodo actual
        if(E == inf){ // lo comparo con el valor buscado
            *aux = A; // si es igual, guardo la dirección del nodo en aux
        }
        buscar(izq(A),inf,aux); // si no es igual, busco en el subárbol izquierdo
        if (*aux==NULL) { // si no lo he encontrado en el izquierdo
            buscar(der(A),inf,aux); // busco en el derecho
        }
    }
}